  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50">
      <StaffHeader
        ...
        section="School Fees & Payments"
      />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-24">
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-8">💰 Complete Payment Records</h1>

        {/* Statistics Cards */}
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 mb-8">
          <div className="bg-white rounded-lg shadow p-3 sm:p-6">
            <p className="text-gray-600 text-xs sm:text-sm">Transactions</p>
            <p className="text-2xl sm:text-3xl font-bold text-green-600">{stats.totalTransactions}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-3 sm:p-6">
            <p className="text-gray-600 text-xs sm:text-sm">Total Collected</p>
            <p className="text-lg sm:text-2xl font-bold text-emerald-600">₦{(stats.totalCollected / 1000000).toFixed(1)}M</p>
          </div>
          <div className="bg-white rounded-lg shadow p-3 sm:p-6">
            <p className="text-gray-600 text-xs sm:text-sm">Completed</p>
            <p className="text-2xl sm:text-3xl font-bold text-green-500">{stats.paidCount}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-3 sm:p-6">
            <p className="text-gray-600 text-xs sm:text-sm">Partial</p>
            <p className="text-2xl sm:text-3xl font-bold text-yellow-600">{stats.partialCount}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-3 sm:p-6">
            ...
            <table className="w-full text-xs sm:text-sm">
              <thead className="bg-gradient-to-r from-green-600 to-emerald-600 text-white">
                <tr>
                  ...
                  <th className="px-3 sm:px-6 py-3 text-left font-semibold hidden sm:table-cell">Method</th>
                </tr>
              </thead>
                ...
                {filteredRecords.length === 0 ? (
                  ...
                  filteredRecords.map((record, idx) => (
                    <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-3 sm:px-6 py-3 text-center font-bold text-green-600">
                      ...
                      <td className="px-3 sm:px-6 py-3 text-gray-600 hidden sm:table-cell text-xs">
