...
export default function PrimaryAccountantDashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      // Verify user is PRIMARY Accountant
      if (
        !currentUser ||
        currentUser.role !== 'ACCOUNTANT' ||
        currentUser.school_level !== 'PRIMARY'
      ) {
      ...
      if (currentUser.school_id) {
        ...
        // Load PRIMARY level students only
        ...
        // Load PRIMARY level staff only
        const { data: staffData, error: staffError } = await supabase
          .from('users')
          .select('id, full_name, email, phone, role, school_level')
          .eq('school_id', currentUser.school_id)
          .in('role', ['TEACHER', 'HEAD_TEACHER', 'STAFF'])
          .eq('school_level', 'PRIMARY')
          .order('full_name')
        ...
        // Load transactions for this school
    ...
    return (
      <div className={`min-h-screen bg-gradient-to-br ${bgClass} flex items-center justify-center`}>
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-4 border-green-500 border-t-emerald-500 mx-auto mb-4"></div>
    ...
    <div className={`min-h-screen bg-gradient-to-br ${bgClass}`}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            { label: 'Total Students (Primary)', value: stats.totalStudents, icon: '👨‍🎓', color: 'from-green-500 to-green-600' },
            { label: 'Total Staff (Primary)', value: stats.totalStaff, icon: '👨‍🏫', color: 'from-emerald-500 to-emerald-600' },
            { label: 'Total Transactions', value: stats.totalPayments, icon: '💳', color: 'from-teal-500 to-teal-600' },
            { label: 'Pending Payments', value: stats.pendingPayments, icon: '⏳', color: 'from-yellow-500 to-yellow-600' },
          ].map((stat, idx) => (
            <div
              key={idx}
          ...
          {activeTab === 'overview' && (
            <div className="p-8">
              <div className={`${darkMode ? 'bg-green-900/30' : 'bg-green-50'} rounded-lg p-6`}>
                <p className={`text-sm ${darkMode ? 'text-green-300' : 'text-green-600'} font-semibold mb-2`}>
                  Primary School Accountant Portal
                </p>
                <p className={darkMode ? 'text-gray-300' : 'text-gray-700'}>
                  Welcome! This dashboard shows all PRIMARY level students and staff records for your school.
                  You can view student information, manage staff records, and track transactions.
                </p>
              </div>
            </div>
          )}
